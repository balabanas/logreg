...
class LogisticRegression:
    ...
    def predict_proba(self, X, append_bias=False):
        ...
        ###########################################################################
        prob_class_1 = 1 / (1 + np.exp(-X.dot(self.w)))
        prob_class_0 = 1 - prob_class_1
        y_proba = np.column_stack((prob_class_0, prob_class_1))
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return y_proba
    ...
    def predict(self, X):
        ...
        Inputs:
        - X: N x D array of training data. Each column is a D-dimensional point. ROW?
        ...
        treshold = 0.5
        y_pred = np.where(y_proba[:, 1] >= treshold, 1, 0)        
        ###########################################################################
        #                           END OF YOUR CODE                              #
        ###########################################################################
        return y_pred
    ...
    def loss(self, X_batch, y_batch, reg):
        ...
        loss = 0
        m = X_batch.shape[0]  # sample size
        
        # Compute loss and gradient. Your code should not contain python loops.
        probs = self.predict_proba(X_batch, False)
        prob_class_0 = probs[:, 0]
        prob_class_1 = probs[:, 1]
        
        reg_cost_l2 = reg / m * np.sum(np.square(self.w[:-1]))  # regularization term for loss computation
        cost = y_batch * np.log(prob_class_1) + (1 - y_batch) * np.log(prob_class_0)
        loss = - np.average(cost) + reg_cost_l2
        
        w_zero_bias = np.copy(self.w)
        w_zero_bias[-1] = 0
        reg_dw_l2 = reg * w_zero_bias  # regularization term for gradient computation
        errors = prob_class_1 - y_batch
        dw = (1 / m) * ((X_batch.T @ errors) + reg_dw_l2)
